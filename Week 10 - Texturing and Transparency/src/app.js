...
window.onload = window['initializeStartRendering'];
...
var loadedAssets = {
    ...
    coloredTile: null                         // an imported image
};

// -------------------------------------------------------------------------
function initializeStartRendering() {
    initGL();
    loadAssets(function() {
...
function loadAssets(onLoadedCB) {
    ...
    var filePromises = [
        ...
        loadImage('./data/colored_tile.png')
    ];
    ...
    Promise.all(filePromises).then(function(values) {
        ...
        loadedAssets.coloredTile = values[4];   // from loadImage
...
function createShaders(loadedAssets) {
    ...
    textureShaderProgram.uniforms = {
        ...
        texture2Uniform: gl.getUniformLocation(textureShaderProgram, "uTexture2"),
...
function createScene() {
    ...
    groundGeometry.create(loadedAssets.uvGridImage, loadedAssets.coloredTile);
    ...
    for (var i = 0; i < 3; ++i) {
        ...
        sphereGeometry.create(loadedAssets.sphereJSON, loadedAssets.uvGridImage, loadedAssets.coloredTile);
...
function updateAndRender() {
    ...
    // #10 apply the painter's algorithm
    _paintersSort();
    // #6
...
let _paintersSort = () => {
    ...
    sphereGeometryList.sort((a, b) => {
        // This comparison function returns negative if a > b
        // and positive if a < b. Basically, we are sorting
        // backwards.
        return ~(a.getPosition().subtract(cameraPos).lengthSqr() - b.getPosition().subtract(cameraPos).lengthSqr());
    });
}
